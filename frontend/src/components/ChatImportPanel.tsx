...
export default function ChatImportPanel({ projectId, project, batches, onChanged, onOpenQueue }: ChatImportPanelProps) {
  ...
  const loadInvites = async () => {
    ...
    try {
      ...
      console.error(error);
      setInviteSetupError(getErrorMessage(error));
      alert(`Blad podczas pobierania zaproszen Google Chat: ${getErrorMessage(error)}`);
    } finally {
      setBusyAction(null);
    }
  };
  ...
  const openInviteSetup = async () => {
    ...
    try {
      ...
      console.error(error);
      setInviteSetupError(getErrorMessage(error));
      alert(`Blad podczas otwierania przegladarki do logowania Google Chat: ${getErrorMessage(error)}`);
    } finally {
      setBusyAction(null);
    }
  };
