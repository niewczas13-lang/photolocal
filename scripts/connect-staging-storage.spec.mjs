...
import { connectStagingStorage } from './connect-staging-storage.mjs';

const scriptsDirectory = dirname(fileURLToPath(import.meta.url));
const probeId = 'photolocal-smb-probe-0123456789abcdef0123456789abcdef';
const volumeName = 'photolocal-staging-nas-0123456789abcdef0123456789abcdef';
...
function temporaryDirectory(context) {
  ...
  const directory = mkdtempSync(join(root, 'photolocal staging storage '));
  context.after(() => {
...
test('successful connection publishes a credential-free manifest and retains only the verified read-only volume', async (context) => {
  const outputDirectory = temporaryDirectory(context);
  const fake = dockerSequence([absent, success, success]);
  const report = await connectStagingStorage({ ...credentials, outputDirectory }, { ...fake, probeId, volumeName });
  assert.equal(report.status, 'STAGING_STORAGE_READY');
  assert.equal(report.cleanup, 'CLEAN');
  assert.equal(report.volumeName, volumeName);
  assert.deepEqual(JSON.parse(readFileSync(join(outputDirectory, 'storage.json'), 'utf8')), {
    version: 1, volumeName, containerPath: '/nas', subdirectory: 'Projects',
  });
  assert.deepEqual(readdirSync(outputDirectory), ['storage.json']);
  ...
  assert.ok(config.volumes.remote.driver_opts.o.startsWith('ro,'));
  assert.equal(config.services.probe.volumes[0].read_only, true);
...
test('an existing manifest is never overwritten and prevents any Docker operation', async (context) => {
  const outputDirectory = temporaryDirectory(context);
  writeFileSync(join(outputDirectory, 'storage.json'), 'existing');
  const fake = dockerSequence([]);
  const report = await connectStagingStorage({ ...credentials, outputDirectory }, { ...fake, probeId, volumeName });
  assert.equal(report.status, 'STORAGE_MANIFEST_EXISTS');
  assert.equal(report.cleanup, 'NOT_NEEDED');
  assert.equal(readFileSync(join(outputDirectory, 'storage.json'), 'utf8'), 'existing');
...
for (const [result, status, cleanup] of [
  ...
  [{ ...success, code: 1 }, 'OTHER_ERROR', 'CLEAN'],
...
test('publication collision preserves the other manifest and removes only the newly created volume', async (context) => {
  ...
  const invoke = async (...args) => {
    ...
    if (args[0][0] === 'compose') writeFileSync(join(outputDirectory, 'storage.json'), 'other process');
    return result;
  };
  const report = await connectStagingStorage({ ...credentials, outputDirectory }, { invoke, probeId, volumeName });
  assert.equal(report.status, 'STORAGE_MANIFEST_EXISTS');
  assert.equal(report.cleanup, 'CLEAN');
  assert.equal(readFileSync(join(outputDirectory, 'storage.json'), 'utf8'), 'other process');
  assert.deepEqual(readdirSync(outputDirectory), ['storage.json']);
...
test('publication temp collision never deletes a pre-existing file', async (context) => {
  const outputDirectory = temporaryDirectory(context);
  const temporaryPath = join(outputDirectory, `storage.json.${probeId}.tmp`);
  writeFileSync(temporaryPath, 'existing private file');
  const fake = dockerSequence([absent, success, success, success]);
  const report = await connectStagingStorage({ ...credentials, outputDirectory }, { ...fake, probeId, volumeName });
  assert.equal(report.status, 'STORAGE_MANIFEST_EXISTS');
  assert.equal(readFileSync(temporaryPath, 'utf8'), 'existing private file');
  assert.ok(!existsSync(join(outputDirectory, 'storage.json')));
});
...
test('unsupported credentials and unsafe resource IDs fail before Docker', async (context) => {
  ...
  for (const [input, options, expected] of [
    ...
    const fake = dockerSequence([]);
    const report = await connectStagingStorage({ ...input, outputDirectory }, { ...fake, ...options });
...
function runWrapper(context, helper, existingManifest = false, unicodeOutputDirectory = false) {
  ...
  copyFileSync(join(scriptsDirectory, 'connect-staging-storage.ps1'), join(directory, 'connect-staging-storage.ps1'));
  writeFileSync(join(directory, 'connect-staging-storage.mjs'), helper, 'utf8');
  if (existingManifest) writeFileSync(join(directory, 'storage.json'), 'existing');
...
& (Join-Path $PSScriptRoot 'connect-staging-storage.ps1') -Server '192.0.2.70' -Share 'Photos' -Subdirectory 'Projects' -UserName 'EXAMPLE-PC\\photo user' -OutputDirectory ${psLiteral(outputDirectory)}
exit $LASTEXITCODE
`, 'utf8');
...
process.stdout.write(JSON.stringify({ status: 'STAGING_STORAGE_READY', cleanup: 'CLEAN', probeId: input.probeId, volumeName: input.volumeName, manifestPath: join(input.outputDirectory, 'storage.json') }));
`);
  assert.ok(existsSync(join(result.directory, 'helper-started')));
  assert.match(result.output, /Status\s*:\s*STAGING_STORAGE_READY/);
