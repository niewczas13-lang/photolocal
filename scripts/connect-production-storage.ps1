...
$storageVolumeName = 'photolocal-production-nas-' + [guid]::NewGuid().ToString('N')
try {
  ...
  $storageManifestPath = Join-Path $storageOutputDirectory 'production-storage.json'
  ...
  $storageHelperPath = Join-Path $PSScriptRoot 'connect-production-storage.mjs'
  if (-not (Test-Path -LiteralPath $storageHelperPath -PathType Leaf)) { throw 'HELPER_MISSING' }
  Write-Host 'Produkcja: osobny wolumen do odczytu i zapisu. Test zapisze tylko swoj nowy folder i go usunie. Aplikacja nie zostanie przelaczona.'
  Write-Host 'Haslo pozostanie w konfiguracji wolumenu Dockera. Wpisz je lokalnie.'
  $storageCredential = Get-Credential -UserName $UserName -Message 'Haslo do udzialu SMB (nie do Windows, Romka ani Google)'
  ...
  # Node emits UTF-8 JSON, independently of the Windows console code page.
  ...
  # Windows PowerShell 5 has no StandardInputEncoding; write explicit UTF-8 bytes.
  ...
  # Never forward child stdout/stderr: Docker errors can contain the credential.
  $storageReport = $storageOutput.Result | ConvertFrom-Json
  $storageAllowedStatuses = @('PRODUCTION_STORAGE_READY', 'STORAGE_MANIFEST_EXISTS', 'STORAGE_MANIFEST_FAILED', 'STORAGE_RESOURCE_EXISTS', 'STORAGE_CLEANUP_FAILED', 'MOUNT_ACCESS_DENIED', 'DIRECTORY_ACCESS_DENIED', 'DIRECTORY_MISSING', 'STORAGE_WRITE_DENIED', 'PHOTO_SAMPLE_NOT_FOUND', 'STORAGE_PROBE_ERROR', 'TEST_FOLDER_CLEANUP_REQUIRED', 'IMAGE_MISSING', 'UNREACHABLE', 'UNSUPPORTED', 'TIMEOUT', 'OTHER_ERROR', 'CREDENTIAL_FORMAT_UNSUPPORTED', 'INVALID_INPUT')
  ...
  if ($storageProcess.ExitCode -eq 0 -and $storageReport.status -eq 'PRODUCTION_STORAGE_READY' -and $storageReport.cleanup -eq 'CLEAN') { $storageExitCode = 0 }
} catch {
